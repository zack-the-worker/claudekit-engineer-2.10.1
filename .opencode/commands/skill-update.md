...
## Your mission
Update an existing skill or its reference files in `.opencode/skills/${SKILL}` directory based on the user's prompt. 
...
### Scopes
- Project-scope: Current working project directory (e.g. `.opencode/`)
- User-scope: Home/user directory (e.g. `~/.opencode/`)

## IMPORTANT NOTES:
- ALWAYS make changes to skills in the project-scope `.opencode/skills/` directory (UNLESS you're allowed to).
- DO NOT make any changes to skills in the home/user-scope `~/.opencode/skills/` directory (UNLESS you're allowed to).
