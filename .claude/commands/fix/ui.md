...
2. Use screenshot capture tools along with `gemini-vision` skill to take screenshots of the implemented fix (at the exact parent container, don't take screenshot of the whole page) and use `eyes_analyze` tool to analyze that screenshots to make sure the result matches the design guideline and address all issues.
